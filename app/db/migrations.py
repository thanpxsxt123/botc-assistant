...
def run_migrations():
    ...
    with engine.connect() as conn:
        ...
        if "is_red_herring" not in cols_players:
            conn.execute(text("ALTER TABLE players ADD COLUMN is_red_herring BOOLEAN DEFAULT 0"))
        ...
        if "secret_info" not in cols_players:
            conn.execute(text("ALTER TABLE players ADD COLUMN secret_info TEXT"))
        if "has_used_ability" not in cols_players:
            conn.execute(text("ALTER TABLE players ADD COLUMN has_used_ability BOOLEAN DEFAULT 0"))
