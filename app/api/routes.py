...
async def get_player_info(session_id: int, name: str, db: Session = Depends(get_db)):
    ...
    if player.character and session:
        ...
        # 4.1 Fortune Teller Info
            ...
            # ดักจับ NightAction ล่าสุดของคืนนี้
            ...
            if last_act:
                ...
                if target1 and target2:
                    ...
                    if player.is_poisoned or player.is_drunk:
                        is_demon = random.choice([True, False]) # ข้อมูลมั่ว
        ...
        # 6. Librarian Info (First Night only)
            ...
            if outsiders:
                ...
                secret_info = "📖 ไม่มีผู้เล่นชาวบ้านนอก (Outsider) อยู่ในเกมเลย"
        ...
        # 7. Investigator Info (First Night only)
            ...
            if minions:
                target_min = random.choice(minions)
                other_players = db.query(Player).filter(
                    Player.session_id == session_id,
                    Player.id != player.id,
                    Player.id != target_min.id
                ).all()
                if other_players:
                    other_p = random.choice(other_players)
                    pair = [target_min.name, other_p.name]
                    random.shuffle(pair)
                    secret_info = f"🕵️‍♂️ 1 ใน 2 คนนี้คือ {target_min.character.name}: {pair[0]} หรือ {pair[1]}"
        ...
        # 9. Ravenkeeper Info
        elif char_name == "Ravenkeeper" and not player.is_alive:
            last_act = db.query(NightAction).filter(
                NightAction.session_id == session_id,
                NightAction.player_id == player.id,
                NightAction.day_number == session.day_number
            ...
            if last_act:
                ...
                if target and target.character:
                    secret_info = f"🦉 ผลการตรวจสอบ: {target.name} คือ {target.character.name}"
            else:
                secret_info = "🦉 โปรดเลือกผู้เล่น 1 คนเพื่อตรวจสอบ (คุณตายคืนนี้จึงได้รับสิทธิ์นี้)"
