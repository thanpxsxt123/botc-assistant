...
async def get_session_by_code(room_code: str, db: Session = Depends(get_db)):
    ...
    if not session:
        # หากไม่มีห้อง ให้สร้างห้องใหม่ทันที
...
async def join_room(data: JoinRoom, db: Session = Depends(get_db)):
    ...
    # 🛡️ ตรวจสอบก่อนว่ามีผู้เล่นชื่อนี้อยู่ในห้องแล้วหรือยัง (ป้องกันชื่อซ้ำเมื่อ Refresh)
    ...
    if existing_player:
        # หากมีอยู่แล้ว ให้ใช้คนเดิม (Re-join)
        player = existing_player
    else:
        # หากเป็นคนใหม่จริง ให้คำนวณเลขที่นั่งและเพิ่มเข้า DB
    ...
    # 🚀 บรอดแคสต์บอกทุกคน (รวมถึง Storyteller) ให้อัปเดตรายชื่อ
    players = db.query(Player).filter(Player.session_id == data.session_id).all()
    player_list = []
    for p in players:
        player_list.append({
            "id": p.id,
...
async def get_player_info(session_id: int, name: str, db: Session = Depends(get_db)):
    ...
    # 🔍 เตรียมข้อมูลลับ (Secret Info) สำหรับฝั่ง Evil
    secret_info = ""
    if player.character:
        # หากเป็น Demon (Imp) ให้เห็นรายชื่อสมุน
        if player.character.char_type == CharacterType.DEMON:
            ...
            if minions:
                m_names = ", ".join([m.name for m in minions])
                secret_info = f"😈 สมุนของคุณคือ: {m_names}"
        
        # หากเป็น Minion ให้เห็น Demon และเพื่อนสมุน
            ...
            info_parts = []
            if demon: info_parts.append(f"🔥 ปีศาจคือ: {demon.name}")
            if other_minions:
                om_names = ", ".join([m.name for m in other_minions])
                info_parts.append(f"👥 สมุนคนอื่น: {om_names}")
            secret_info = " | ".join(info_parts)

    return {
        "player_id": player.id,
        "name": player.name,
        "character": player.character.name if player.character else "Unknown",
        "alignment": player.character.alignment.value if player.character else "Unknown",
        "is_alive": player.is_alive,
        "is_drunk": player.is_drunk,
        "is_poisoned": player.is_poisoned,
        "ability": player.character.ability_description if player.character else "",
...
async def narrate_text(data: dict, db: Session = Depends(get_db)):
    ...
    # บอร์ดแคสต์ให้ผู้เล่นทุกคนเล่นเสียงนี้
