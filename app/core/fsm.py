...
class GameEngine:
            ...
            if highest.votes_received >= threshold:
                ...
                if len(ties) == 1:
                    ...
                    if executed_player.character.name == "Saint" and not executed_player.is_drunk and not executed_player.is_poisoned:
                        session.current_phase = GamePhase.GAME_OVER
                        self.log_action(session_id, GamePhase.GAME_OVER, session.day_number, "Evil wins! Saint was executed.")
                else:
                    self.log_action(session_id, session.current_phase, session.day_number, 
    ...
    def resolve_night_actions(self, session_id: int):
        ...
        living_demon = db.query(Player).join(Character).filter(
        ...
        if not living_demon:
            living_players_count = db.query(Player).filter(Player.session_id == session_id, Player.is_alive == True).count()
            if living_players_count >= 5:
                sw = db.query(Player).join(Character).filter(
                    Player.session_id == session_id,
                    Player.is_alive == True,
