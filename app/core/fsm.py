...
class GameEngine:
    ...
    def check_win_conditions(self, session_id: int):
        ...
        # 3. Mayor Win Condition (Wait until night or end of day with 3 left)
        if len(living_players) == 3:
            # Check if Mayor is alive and not drunk/poisoned
            mayor = next((p for p in living_players if p.character.name == "Mayor" and not p.is_drunk and not p.is_poisoned), None)
            if mayor:
                 # Standard rules: If only 3 left and no execution, Mayor wins
                 ...
                 if not executed_today and session.current_phase == GamePhase.NIGHT:
                     session.current_phase = GamePhase.GAME_OVER
                     self.log_action(session_id, GamePhase.GAME_OVER, session.day_number, "🏆 บทสรุปเกม: ฝั่งดีชนะ! เนื่องจากพลังของ Mayor (เหลือ 3 คนและไม่มีใครตาย)")
