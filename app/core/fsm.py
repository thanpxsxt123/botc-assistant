...
class GameEngine:
    ...
    def assign_characters(self, session_id: int):
        ...
        # 🚀 สุ่ม Red Herring สำหรับ Fortune Teller
        townsfolk_players = [p for p in players if p.character and p.character.char_type == CharacterType.TOWNSFOLK and not p.is_drunk]
        if townsfolk_players:
            red_herring = random.choice(townsfolk_players)
    ...
    def start_game(self, session_id: int):
        ...
        if session.current_phase == GamePhase.GAME_OVER:
            ...
            for p in session.players:
                ...
                p.is_poisoned = False
                p.is_drunk = False
                p.is_red_herring = False
                p.secret_info = None
                p.has_used_dead_vote = False
                p.has_voted_this_day = False
    ...
    def check_win_conditions(self, session_id: int):
        ...
        # 🏆 Mayor Win Condition: If no execution today and only 3 players left (and one is Mayor)
        if len(living_players) == 3 and session.day_number >= 3:
            mayor = next((p for p in living_players if p.character.name == "Mayor" and not p.is_drunk and not p.is_poisoned), None)
            if mayor:
                 # In a real game, this usually requires "no execution today". 
                 # We can check if any execution happened today.
                 ...
                 if not executed_today and session.current_phase == GamePhase.NIGHT:
                     session.current_phase = GamePhase.GAME_OVER
        ...
        if nominations:
            ...
            if highest.votes_received >= threshold:
                ...
                if len(ties) == 1:
                    ...
                    self.db.commit()
                    summary_msg = f"⚖️ บทสรุปการประหาร: {executed_player.name} ถูกประหารชีวิตด้วยคะแนน {highest.votes_received} เสียง"
                    self.log_action(session_id, session.current_phase, session.day_number, summary_msg)
                    
                    if executed_player.character.name == "Saint" and not executed_player.is_drunk and not executed_player.is_poisoned:
                        session.current_phase = GamePhase.GAME_OVER
                        self.log_action(session_id, GamePhase.GAME_OVER, session.day_number, "🏆 บทสรุปเกม: ฝั่งร้ายชนะ! เนื่องจาก Saint ถูกประหารชีวิต")
                else:
                    summary_msg = "⚖️ บทสรุปการประหาร: ไม่มีใครถูกประหารเนื่องจากคะแนนโหวตสูงสุดเท่ากัน (Tie)"
                    self.log_action(session_id, session.current_phase, session.day_number, summary_msg)
            else:
                summary_msg = f"⚖️ บทสรุปการประหาร: ไม่มีใครถูกประหารเนื่องจากคะแนนไม่ถึงเกณฑ์ ({highest.votes_received}/{threshold})"
                self.log_action(session_id, session.current_phase, session.day_number, summary_msg)
        else:
            summary_msg = "⚖️ บทสรุปการประหาร: ไม่มีใครถูกเสนอชื่อประหารชีวิตในวันนี้"
            self.log_action(session_id, session.current_phase, session.day_number, summary_msg)
